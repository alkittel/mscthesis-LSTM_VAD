...
if ~nargout
    ...
    legend(['Equiv Noise BW = ' v_sprintsi(x.enbw,-2) 'cyc/L'],['Max sidelobe = ' sprintf('%.0f',x.sidelobe) ' dB'],['-3 & -6dB BW = ' sprintf('%.2g',(x.band3)) ' & ' sprintf('%.2g',(x.band6)) ' cyc/L']);
    hold off
    ...
    v_texthvc(tax(end),max(w),sprintf('N=%d',nw),'rtk');
    if length(x.cola)>3
