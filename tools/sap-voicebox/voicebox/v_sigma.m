...
%
%   External Functions:
%       Function v_gaussmix in Voicebox.
%
%   Notes:
...
% Determine clusters
[mm,vv,ww]=v_gaussmix(nfv,var(nfv)/100,[],nclust,'kf');
...
for i=1:nclust
    [m_,v_,w_,g,f,ll(i,:)]=v_gaussmix(nfv,[],0,mm(i,:),vv(i,:),ww(i,:));
end
...
% Determine clusters
[mm,vv,ww]=v_gaussmix(pfv,var(pfv)/100,[],nclust,'kf');
...
ll = [];
for i=1:nclust
    [m_,v_,w_,g,f,ll(i,:)]=v_gaussmix(pfv,[],0,mm(i,:),vv(i,:),ww(i,:));
end
[m,in]=max(ll); % Find which cluster each feature vector belongs to
...
if(nargin < 4)
    fwlen = v_voicebox('dy_fwlen');          % window length to smooth group delay
end
if (nargin < 3)
    gwlen = v_voicebox('dy_gwlen');          % window length of group delay
end
