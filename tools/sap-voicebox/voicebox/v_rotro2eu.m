...
%V_ROTRO2EU converts a 3x3 v_rotation matrix into the corresponding euler angles
% inverse of v_roteu2ro
%     M        a string of n characters from the set determining the order of v_rotation axes
...
%                                  'A' = axes-intrinsic
%
%     R(3,3,...)   Input v_rotation matrix (or array of matrices)
%
% Outputs:
%
%     E(K,...)  K Euler angles in radians (or degrees if 'd' or 'D' specified) per quaternion where K is the number of 'xyz' characters in M.
%               A positive v_rotation is clockwise if looking along the +ve axis away from the origin or anti-clockwise if 'R' or 'D' is given.
...
if isempty(mvch)
    mch=''; % cached v_rotation code string
    mvch=[0;0;1;1;0;0;1];
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    if ischar(m) && strcmp(m,mch) % check if the v_rotation code string is cached
        mv=mvch;
    else
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    nm=size(mv,2)-1; % number of v_rotation codes
    sz=size(r);
    r=reshape(r,9,[]); % vectorize the v_rotation matrices
    nr=size(r,2);                       % number of v_rotation matrices
    if mv(end)<0
        r=r(rtr,:);                     % transpose v_rotation matrix
    end
    ...
    for i=nm:-1:1                       % process rotations in reverse order
        mvi=mv(:,i);
        mi=mvi(1);
        if mi<=3                        % v_rotation around x,y or z
            if mvi(6)~=0               % skip if this v_rotation is redundant
                ...
                r(rtci(:,mi),:)=ci(w6,:).*r(rtci(:,mi),:)-si(x6,:).*r(rtsi(:,mi),:); % apply reverse v_rotation
            end
        else % fixed v_rotation
            ai=scai(4,mi);                      % axis of v_rotation: 1, 2 or 3
            r(rtci(:,ai),:)=scai(th6,mi).*r(rtci(:,ai),:)-scai(x6,mi).*r(rtsi(:,ai),:); % apply reverse v_rotation
        end
    end
