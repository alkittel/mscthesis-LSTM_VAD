...
if isstruct(fsx)
    ...
    elseif any(m=='A')
        ...
        if any(m=='e')
            ...
            se=v_activlev(sf,fs);     % or else P.56
        end
    else
        if any(m=='e')
            se=mean(s.^2);          % use normal energy
        else
            se=v_activlev(s,fs);      % or else P.56
        end
    end
    ...
    if genf   % generate noise locally
        ...
        elseif any(m=='A')
            ...
            if any(m=='P')
                ne=v_activlev(sf,fs);     % use P.56 for noise power
            else
                ne=mean(sf.^2);         % or just normal energy
            end
        else
            if any(m=='P')
                ne=v_activlev(nb,fs);     % use P.56 for noise power
            else
...
if ~isstruct(fsx) && (~nargout || any(m=='g'))
    ...
    v_texthvc(tax(1),zp,sprintf(' %.1f dB',lg),'lbb');
    v_axisenlarge([-1.005 -1.05]);
    ylim=get(gca,'ylim');
    if snre==0
        lg=-Inf;
    elseif snre==Inf
    ...
    v_texthvc(tax(ns),ylim(2),sprintf('%.1f dB SNR ',lg),'rtb');
    xlabel('Time (s)');
    ylabel('Output');
    subplot(3,1,2)
    if nze==0
        plot(tax([1 ns]),[0 0],'-y');
        v_axisenlarge([-1.005 1]);
    else
        ...
        v_texthvc(tax(1),sqrt(pe(2)),sprintf(' %.1f dB',lg),'lbb');
        v_axisenlarge([-1.005 -1.05]);
        ylim=get(gca,'ylim');
        if pe(4)/pe(2)==0
            lg=-Inf;
        ...
        v_texthvc(tax(ns),ylim(2),sprintf('\\times %.1f dB ',lg),'rtb');
    end
    ylabel('Noise');
    subplot(3,1,1)
    ...
    v_texthvc(tax(1),sqrt(pe(1)),sprintf(' %.1f dB',lg),'lbb');
    v_axisenlarge([-1.005 -1.05]);
    ylim=get(gca,'ylim');
    if pe(3)/pe(1)==0
        lg=-Inf;
