...
%V_ROTEUCODE decodes a string specifying a v_rotation axis sequence
%     M(n)     a string of n characters from the set determining the order of v_rotation axes
...
%                               whether the v_rotation axes remain fixed in space for consecutive
%                               rotations (extrinsic) or else move with each v_rotation (intrinsic).
...
%                    mv(1,j) = Code for the j'th v_rotation: 1,2,3 for x,y,z v_rotation and 4 to 12 for the fixed rotations listed above.
...
%                    mv(3,j) = v_rotation class before v_rotation j. mv(3,k) is the final v_rotation class and equals 52 for arbitrary rotations.
%                    mv(4,j) = index into a vectorized matrix of the entry that becomes non-zero after v_rotation j
%                    mv(5,j) = index into a vectorized matrix of the other changing element in the same column
%                    mv(6,j) = +-1 = sign of the sine term affecting entry mv(4,j). For mv(1,j) in [1,3], mv(6,j)=0 if the v_rotation is unnecessary
%                    mv(7,j) = +-1 = sign of entry mv(5,j) before v_rotation j if known
%                Special entries:
%                    mv(7,k) = -1 to invert the v_rotation (i.e. transpose the matrix) or +1 otherwise
...
if isempty(mes)     % setup fixed arrays and initialize cache of mode strings
    ...
    % patterns of -1,0,+1 that may exist in a v_rotation matrix as follows:      %
    %                                                                          %
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    % trmap(i,j) gives the pattern that i is transformed into by v_rotation j    %
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    % Each Euler angle is chosen so that the inverse v_rotation forces a specific element  %
    % of the v_rotation matrix to zero. zel(k,j,i) gives information about which element   %
    % ceases to be zero when a v_rotation around axis j is applied to pattern i.           %
...
if isempty(ich)                 % not yet in the cache
    ...
    es=1;                       % angle sign
    fl=1;                       % default to no v_rotation matrix tranposing
    for i=1:length(mc)
        switch mc(i)
            case -5             % 'r' = radians
            case -19            % 'd' = degrees
    ...
    for i=1:nm                % loop for each v_rotation
        mmi=mm(i); % v_rotation code
        mv(3,i+1)=trmap(mv(3,i),mmi); % pattern ID after v_rotation
        if mmi<4
